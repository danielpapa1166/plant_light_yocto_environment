...
#define MAX_USER_BUFFER_SIZE 1024
#define BCM2711_GPIO_ADDRESS 0xfe200000

static struct proc_dir_entry *proc = NULL;


static char data_buffer[MAX_USER_BUFFER_SIZE + 1] = {0};
...
static void gpio_pin_off(unsigned int pin)
{
	...
	*gpio_off_register |= (1 << pin);
    ...
    pr_info("GPIO off register %08X \n", * gpio_off_register); 

	return;
}


static ssize_t read(struct file *file, char __user *user, size_t size, loff_t *off)
{
	return copy_to_user(user,"Hello!\n", 7) ? 0 : 7;
}

static ssize_t write(struct file *file, const char __user *user, size_t size, loff_t *off)
{
...
static int __init gpio_driver_init(void)
{
	...
	proc = proc_create("gpio_chardev", 0666, NULL, &proc_fops);
	if (proc == NULL)
	{
		return -1;
	}
